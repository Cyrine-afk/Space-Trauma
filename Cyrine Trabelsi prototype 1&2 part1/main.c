...
/**** load stage ****/
LoadLevel("level.txt",&niveau);

/**** load jeu ****/
LoadGame("SaveGame.txt",&h,/*&pos,*/&tmps,&enmy);
...
imgfin=IMG_Load("doura station.png");
...
if (b[5]==1)
{
 afficher_save_escape(&sv,Tab,&msc);
}
...
if (flag_save ==1)
{
 if (SAVE==0)
 {
   afficher_save (&sv,Tab,motion_a,motion_b,&msc);
   ...
   if(t==0)
   {
    ...
    if(b[0])
    {
     if(h.position.x>=200)
     {
      ...
      /*if(h.position.x>=2662 && h.position.y>=495)
       {
         continuer =0;
       }*/
     }
...
/**** gestion de score et de vie ****/
scoree2 (&h,&d,Tab,location2,relativ6);
...
SDL_Rect rltvv ={posfin.x-a,posfin.y-c}; //end
SDL_BlitSurface(imgfin,NULL,Tab[0],&posfin);
...
/*if (end==0)
{
    if(colision(&location2,&posfin))
    {
      if(cols(h.position,rltvv))
      {
       continuer=0;
       end=1;
      }
